...
public:
    ...
    ~Vector(){
        Clear();  // call every destructor
        // delete[] m_Data;  // It may delete twice
        ::operator delete[](m_Data);  // do not call the destructor of every elements
    }
private:
    void ReAlloc(size_t newCapacity){
        // allocate new block of memory
        // T* newBlock = new T[newCapacity];  // call the default constructor
        T* newBlock = (T*)::operator new(newCapacity*sizeof(T)) ;  // do not call the default constructor
        ...
        for (size_t i = 0; i < m_Size; i++)
            ...
            new(&newBlock[i]) T(std::move(m_Data[i]));  // placement new, it will call constructor
        
        // delete old block
        // delete[] m_Data;  // call the destructor of every elements
        for (size_t i = 0; i < m_Size; i++)
        {
            m_Data[i].~T();
        }
